...
# ============================================================
FIGURE_NAME = 'CD4+T'          # ← 图片标题（显示在图表上方）
...
data0 = df[col_names[0]].dropna().values
data1 = df[col_names[1]].dropna().values
...
# 自动计算步长，使Y轴刻度在4~6之间
magnitude = 10 ** int(np.floor(np.log10(data_max)))
step = None
y_max = None
for base in [1, 2, 5, 10]:
    for scale in [1, 0.1]:
        s = base * magnitude * scale
        if s < 1:
            continue
