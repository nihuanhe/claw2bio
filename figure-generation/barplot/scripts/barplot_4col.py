...
# ============================================================
FIGURE_NAME = 'sh-NOXA'          # ← 图片标题（显示在图表上方）
Y_LABEL     = 'MFI'              # ← Y轴标签（显示在Y轴左侧）
...
# ===== 图表布局 =====
# 使用与 3-column 相同的 inch_per_xunit，确保柱子物理尺寸一致
# 4柱: x_range = 13/7, 需要 axes_width = 3.6031 * 13/7 ≈ 6.691
fig, ax = plt.subplots(figsize=(9.0, 6))
plt.subplots_adjust(left=0.060, right=0.8035, bottom=0.064537, top=0.975000)

bar_width = 1.7 / 7
# 4组数据，x_pos 按 7等分网格：每组分 3/7，左/右边距各 2/7
n_pos = 4
x_pos = np.array([2/7 + i * 3/7 for i in range(n_pos)])

bar_colors   = ['#0072B2', '#D55E00', '#009E73', '#CC79A7']
point_colors = ['#80FFFF', '#EDA461', '#90EE90', '#D683F1']
...
output_path = DATA_FILE.replace('.csv', '_barplot4col.png')
