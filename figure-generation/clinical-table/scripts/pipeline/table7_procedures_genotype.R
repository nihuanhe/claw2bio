# [Verbatim from the CRE 67-isolate reproduction pack sec 4.8. Changes vs the reproduction pack: header comment (ASCII English) + Rscript path neutralized + table renamed to the skill-wide Table1-10 scheme (manuscript number kept alongside). Inputs are the pseudonymized frozen CSVs.]
# encoding: UTF-8
# ----------------------------------------------------------------------------
# Table 7_procedures_genotype script (manuscript Table 7_procedures_genotype): invasive procedures / low albumin / length of
# hospital stay x carbapenemase groups
# Run: Rscript table7_procedures_genotype.R
# Purpose: read CRE data -> group by Bla_Carb_acquired -> compute n (%) for each
#          feature (Total column denominator = 67; low albumin = Albumin column
#          equals 0) -> fill the Table 7_procedures_genotype section of Table-all.md
#          -> write table7_procedures_genotype.csv
# ----------------------------------------------------------------------------
...
# Each feature and its "positive" value (low albumin: Albumin column 0 = <=35 g/L)
factors <- c("Urinary catheterization" = "Urinary_cath",
             "Endotracheal intubation" = "Endo_intub",
             "Gastric tube" = "Gastric_tube",
             "Low albumin level (\u226435 g/L)" = "Albumin",
             "Hospital stay >7 days" = "Hospital_stay")
present <- c(Urinary_cath = 1L, Endo_intub = 1L, Gastric_tube = 1L,
             Albumin = 0L, Hospital_stay = 1L)

# ---------------- Read S5 template row labels from Table-all.md ----------------
src <- readLines(MD_FILE, encoding = "UTF-8")
h <- grep("^## Table 7_procedures_genotype \\(revised\\)", src)
...
rows <- lapply(names(factors), function(lab) {
  col  <- cl[[factors[[lab]]]]
  val  <- present[[factors[[lab]]]]
  cnt  <- vapply(grp_order, function(g) sum(grp == g & col == val), integer(1))
  c(lab,
    vapply(grp_order, function(g) fmt_cell(cnt[[g]], grp_n[[g]]), character(1)),
    fmt_cell(sum(cnt), 67))
})

# ---------------- Replace the S5 row block in Table-all.md ----------------
...
write.csv(csv_df, "table7_procedures_genotype.csv", row.names = FALSE, fileEncoding = "UTF-8")

cat("Table 7_procedures_genotype done: filled into Table-all.md and written to table7_procedures_genotype.csv\n")
