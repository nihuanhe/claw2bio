...
mtDNA qPCR 批量处理脚本
======================
遍历指定目录下所有 CSV，按文件夹名作为 Figure 名，自动调用 run_mtdna.py 中的处理逻辑。

用法：
    python batch_mtdna.py "D:/小文章/教学示范/整理后的数据-PCR/new" --overwrite
"""
...
from run_mtdna import process_one, Y_LABEL_DEFAULT, DPI_DEFAULT


def main():
    parser = argparse.ArgumentParser(
        description="批量处理 mtDNA qPCR CSV",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python batch_mtdna.py "D:/data/mtDNA-PCR" --overwrite
  python batch_mtdna.py "D:/data/mtDNA-PCR" --y-label "mtDNA copy number" --dpi 300
""",
    )
