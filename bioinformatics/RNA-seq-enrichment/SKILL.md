...
**Prerequisite / 前提**: run `bioinformatics/bulk-RNA-seq` first (counts → DEG).
...
- Reactome cache: open-license, redistributable (mirrored on COS at launch).
...
- The OrgDb packages are archived in the bulk-RNA-seq skill's `resources/`
  (COS mirror at launch) — one OrgDb install serves both skills.
